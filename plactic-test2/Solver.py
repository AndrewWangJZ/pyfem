...
class Solver(object):
    ...
    def displacementBoundaryCondition(self, u_value, flag):
        if flag == 0:
    ...
    def solveStiffFuction(self, u_value, flag=1):
        ...
        self.displacementBoundaryCondition(u_value, flag)
    ...
    def solve(self):
        ...
        for i in range(self.n_step):
            ...
            for elem in self.elements:
                ...
                elem.PlasticIteration(d_u_elem)
                elem.updateElementStiffness(d_u_elem)
                # for gauss_i in elem.gauss_points:
                #     print(gauss_i.mat.yieldFunction(gauss_i.sigma))
            self.stiffnessAssembling()
            #######################################
            self.u_calculated.append(d_u + self.u_calculated[-1])
            self.f_calculated.append(d_f + self.f_calculated[-1])
            # self.psi = self.f_calculated[-1] - \
            #            np.einsum("ijkl, kl->ij", self.K_global,
            #                      self.u_calculated[-1])
            # print(self.psi)
            V_0 = 0
            for elem in self.elements:
